...
def _run_git(*args):
    return subprocess.run(['git', *args], cwd=_here,
                          capture_output=True, text=True, check=True).stdout.strip()

_bi = {'commit': None, 'commit_full': None, 'commit_date': None,
       'commit_subject': None, 'dirty': None,
       'build_time': datetime.datetime.now(datetime.timezone.utc).isoformat()}
try:
    _bi['commit'] = _run_git('rev-parse', '--short', 'HEAD')
    _bi['commit_full'] = _run_git('rev-parse', 'HEAD')
    _bi['commit_date'] = _run_git('log', '-1', '--format=%cI')
    _bi['commit_subject'] = _run_git('log', '-1', '--format=%s')
    # 只统计已跟踪文件的改动 (对齐 git describe --dirty 语义), 未跟踪文件不误报。
    _status = subprocess.run(['git', 'status', '--porcelain', '--untracked-files=no'],
                             cwd=_here, capture_output=True, text=True, check=True).stdout
    _bi['dirty'] = bool(_status.strip())
except Exception as _e:
    print(f'[build.spec] git info unavailable, fallback to None: {_e}')
    for _k in ('commit', 'commit_full', 'commit_date', 'commit_subject'):
        _bi[_k] = None

try:
    with open(os.path.join(_here, '_build_info.py'), 'w', encoding='utf-8') as _f:
        _f.write('# This file is auto-generated by build.spec at build time. Do not edit.\n')
        for _k, _v in _bi.items():
            _f.write(f'{_k} = {_v!r}\n')
except Exception as _e:
    print(f'[build.spec] failed to write _build_info.py: {_e}')
...
a = Analysis(
    ...
    binaries=binaries,
    datas=datas,
    hiddenimports=collect_submodules('pykep') + collect_submodules('orbcalc') + ['webapp', '_build_info'],
    hookspath=[],
    hooksconfig={},
    runtime_hooks=[],
    excludes=[],
