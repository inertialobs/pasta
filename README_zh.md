...
- **全局计算配置**：只有一份计算配置（流水线开关/保留数/进程数/冒烟），每次提交任务自动更新、重启自动恢复——计算参数不再挂在预设上。
...
2. **计算配置**：流水线 4 阶段开关、冒烟/完整模式、并行进程数、搜索步进、扫描/细化保留数。
...
6. **预设**：任务预设下拉，载入即填任务表单（带加载反馈），「存为…」导出到 `presets/*.json`。计算配置为全局唯一一份（`orbitcalculator.compute.json`），每次提交自动保存。
